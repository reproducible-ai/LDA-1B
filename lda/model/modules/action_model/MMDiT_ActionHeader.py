...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        if self.vision_encoder_type == "dinov3":
            ...
            print("###########################################")
            self.vision_encoder = VAEDownsample()
        ...
        if self.vision_encoder_type == 'vae':
            ...
            latent_patch_shape = config.patch_shape
            self.obs_patchifier = MultiViewVideoPatchifier(
                    num_views=config.num_views,
                    # input_shape=config.img_shape,
                    time_horizon=self.obs_horizon + 1,
            ...
            self.obs_merger = nn.Linear(num_chans * total_obs_len, self.input_embedding_dim)
    ...
    def to_tokens(self, obs, timestep=None):
        ...
        if self.glob_len > 0:
            ...
            glob_tokens = self.obs_patchifier.proj_glob(
                obs[..., :-self.patch_len].permute(0, 1, 3, 4, 2).reshape(len(local_tokens), self.glob_len * self.obs_horizon * self.num_views, -1)  # for glob tokens, channelwise concat
            )
    ...
    def get_next_obs_tokens(self, image_tokens, obs_indices):
        if self.vision_encoder_type == 'vae':
    ...
    def forward(
        ...
        Args:
            ...
            history_actions: (B, T, action_dim) [optional]
        ...
        if self.vision_encoder_type == "vjepa2":
            curr_obs = rearrange(curr_obs, "(b v) t h w c -> b (v h w) (c t)", b=B, v=V)
        elif self.vision_encoder_type == "dinov3":
            curr_obs = rearrange(curr_obs, "(b v t) n c -> b (v n) (c t)", b=B, v=V)
        elif self.vision_encoder_type == "vae":
        ...
        if self.vision_encoder_type == "vae":
            policy_obs_feat = self.next_obs_learnable_tokens.unsqueeze(0).unsqueeze(0).unsqueeze(0).unsqueeze(0).unsqueeze(0).permute(0, 1, 5, 2, 3, 4).expand(len(policy_indices), V, -1, next_obs.shape[3], H, W)
        else:
        ...
        if self.vision_encoder_type == "vae":
            obs_tokens = self.to_tokens(torch.cat([curr_obs, noisy_next_obs], dim=3), diffusion_t) # (B, V, C, T, H, W)
        else:
        ...
        total_loss += policy_act_loss
        obs_loss = torch.tensor(0)
        if self.policy_and_video_gen:
            pred_next_obs = self.get_next_obs_tokens(image_tokens, video_gen_indices)            
            ...
            total_loss += inverse_act_loss
            pred_next_obs = self.get_next_obs_tokens(image_tokens, pred_next_obs_task_indices)
            obs_loss = F.mse_loss(pred_next_obs[:len(forward_dynamics_indices)], obs_velocity[:len(forward_dynamics_indices)])
            total_loss += obs_loss
    ...
    def predict_action(
        ...
        for step in range(num_steps):
            ...
            if self.vision_encoder_type == "vae":
                obs_tokens = self.to_tokens(torch.cat([curr_obs, noisy_next_obs], dim=3), t_discretized) # (B, V, C, T, H, W)
            else:
                obs_tokens = self.obs_merger(torch.cat([curr_obs, noisy_next_obs], dim=-1))
            # === 5.4 Assemble full input sequence (same order as forward) ===
            if state_features is not None:
                action_features = torch.cat([state_features, action_features], dim=1)
            if history_action_features is not None:
                action_features = torch.cat([history_action_features, action_features], dim=1)
    ...
    def video_gen(
        ...
        for step in range(num_steps):
            ...
            register_tokens = self.register_tokens.weight.unsqueeze(0).expand(B, -1, -1)  # (B, T_f, D)
            if self.vision_encoder_type == "vae":
                obs_tokens = self.to_tokens(torch.cat([curr_obs, next_obs], dim=3), t_discretized) # (B, V, C, T, H, W)
            else:
                obs_tokens = self.obs_merger(torch.cat([curr_obs, next_obs], dim=-1))
            # === 5.4 Assemble full input sequence (same order as forward) ===
            if state_features is not None:
                action_features = torch.cat([state_features, action_features], dim=1)
            if history_actions is not None:
            ...
            pred_velocity = self.obs_projector(image_tokens)
